...
from data.noisy_cifar_open import AddOpenNoisyCIFAR100 
...
def build_transform(rescale_size=512, crop_size=448):
    ...
    cifar10_train_transform_strong_aug = torchvision.transforms.Compose([
        torchvision.transforms.RandomCrop(32, padding=4),
        torchvision.transforms.RandomHorizontalFlip(),
    ...
    return {'train': train_transform, 'test': test_transform, 'train_strong_aug': train_transform_strong_aug,
            ...
            'cifar10_train': cifar10_train_transform, 'cifar10_test': cifar10_test_transform, 'cifar10_train_strong_aug': cifar10_train_transform_strong_aug}
...
def build_cifar100n_open_train_dataset(root, train_transform, test_transform, noise_type, openset_ratio, closeset_ratio, ood_noise_name, ood_noise_root_dir, ood_noise_num_train, ood_noise_num_test):
    train_data = AddOpenNoisyCIFAR100(root, train=True, transform=train_transform, download=False, noise_type=noise_type, closeset_ratio=closeset_ratio,openset_ratio=openset_ratio, verbose=True, ood_noise_name=ood_noise_name, ood_noise_root_dir=ood_noise_root_dir, ood_noise_num=ood_noise_num_train)
    test_data = NoisyCIFAR100(root, train=False, transform=test_transform, download=False, noise_type='clean', closeset_ratio=closeset_ratio, openset_ratio=openset_ratio, verbose=True)
    return {'train': train_data, 'test': test_data, 'n_train_samples': len(train_data.data), 'n_test_samples': len(test_data.data)}


def build_cifar100n_open_all_dataset(root, train_transform, test_transform, noise_type, openset_ratio, closeset_ratio, ood_noise_name, ood_noise_root_dir, ood_noise_num_train, ood_noise_num_test):
    train_data = AddOpenNoisyCIFAR100(root, train=True, transform=train_transform, download=False, noise_type=noise_type, closeset_ratio=closeset_ratio,openset_ratio=openset_ratio, verbose=True, ood_noise_name=ood_noise_name, ood_noise_root_dir=ood_noise_root_dir, ood_noise_num=ood_noise_num_train)
    train_used_idx = train_data.openset_select_ids
    test_data = AddOpenNoisyCIFAR100(root, train=False, transform=test_transform, download=False, noise_type='clean', closeset_ratio=closeset_ratio,openset_ratio=openset_ratio, verbose=True, ood_noise_name=ood_noise_name, ood_noise_root_dir=ood_noise_root_dir, ood_noise_num=ood_noise_num_test, train_used_idx=train_used_idx)
    return {'train': train_data, 'test': test_data, 'n_train_samples': len(train_data.data), 'n_test_samples': len(test_data.data)}
