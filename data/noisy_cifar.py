...
class NoisyCIFAR100(torchvision.datasets.CIFAR100):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False,
        ...
        self.open_cls = list(range(int(self.nb_classes * (1 - openset_ratio)), self.nb_classes))
        if self.train and (noise_type != 'clean'):
            ...
            self.is_clean = np.transpose(self.noisy_labels) == np.transpose(self.targets)
            # print(f'self noise rate: {(np.array(self.noisy_labels) != np.array(self.targets)).mean()}')
            # self.open_cls = list(range(int(self.nb_classes * (1 - openset_ratio)), self.nb_classes))
        else:
            # new_data = []
            # new_targets = []
            ...
            print('Do not exclude the open set class!')
            pass
    ...
    def __getitem__(self, index):
        ...
        if self.noise_type != 'clean':
            res = {'index': index, 'data': img, 'label': target, 'clean_label': self.targets[index], 'is_clean': self.is_clean[index], 'is_open': self.targets[index] in self.open_cls}
        else:
            res = {'index': index, 'data': img, 'label': target, 'clean_label': self.targets[index], 'is_clean': True, 'is_open': self.targets[index] in self.open_cls}
        return res
...
class NoisyCIFAR10(torchvision.datasets.CIFAR10):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False,
        ...
        if self.train and (noise_type != 'clean'):
            ...
            self.noisy_labels = [i[0] for i in noisy_labels]
            train_labels = [i[0] for i in train_labels]
            self.is_clean = np.transpose(self.noisy_labels) == np.transpose(train_labels)
            self.open_cls = list(range(int(self.nb_classes * (1 - openset_ratio)), self.nb_classes))
        else:
            new_data = []
            new_targets = []
    ...
    def __getitem__(self, index):
        ...
        if self.noise_type != 'clean':
            res = {'index': index, 'data': img, 'label': target, 'is_clean': self.is_clean[index], 'is_open': self.targets[index] in self.open_cls}
        else:
            res = {'index': index, 'data': img, 'label': target, 'is_clean': True, 'is_open': False}
        return res
